...
RemoteLS.prototype._request = function (packageName, name, task, callback) {
  var _this = this
  var packageUrl = this.registry.replace(/\/$/, '') + '/' + packageName
...
RemoteLS.prototype._loadPackageJson = function (task, done) {
  ...
  this._request(couchPackageName, name, task, function (err, obj) {
    if (err) {
      return done()
    }

    try {
      if (_this.verbose) console.log('loading:', name + '@' + task.version + ' - ' + task.license)
...
RemoteLS.prototype._walkDependencies = function (task, packageJson, done) {
  ...
  var fullName = packageJson.name + '@' + version + (this.license ?  ' - ' + license : '')
